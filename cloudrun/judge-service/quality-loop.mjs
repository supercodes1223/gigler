/**
 * Ported from amplify/functions/gigler-gig-processor/quality-loop.ts — keep in sync.
 *
...
export function sanitizeReview(raw, actionCount) {
  ...
  const issues = Array.isArray(r.issues)
    ? r.issues.filter((i) => typeof i === "string").slice(0, 10)
    : [];
  ...
  const vetoedActionIndexes = Array.isArray(r.vetoedActionIndexes)
    ? [...new Set(r.vetoedActionIndexes.filter(
        (i) => typeof i === "number" && Number.isInteger(i) && i >= 0 && i < actionCount
      ))]
    : [];
 ...
 */
export async function reviewDraft(params, deps) {
 ...
 */
export async function runQualityLoop(params, deps) {
  const passthrough = {
