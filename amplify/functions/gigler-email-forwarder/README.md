...
A standalone AWS Lambda function that forwards all inbound email sent to `*@gigler.ai` to `support@carmenai.co`.
...
Sender (anyone@anywhere)
  ...
  -> support@carmenai.co (Gmail)
```

## Forwarding behavior

- All addresses at `@gigler.ai` (admin@, support@, hello@, gig@, foo+bar@, anything) forward to `support@carmenai.co`.
...
```js
forwardMapping: {
  "@gigler.ai": ["support@carmenai.co", "alerts@example.com"],
}
```
...
```js
forwardMapping: {
  "admin@gigler.ai": ["support@carmenai.co"],
